...
function createReminderScript(input) {
  const body = [
    ...
    "if (DUE_MS !== null) props.dueDate = new Date(DUE_MS);",
...
function monthWindow(anchorMs) {
  return { fromMs: anchorMs - 7 * DAY_MS, toMs: shiftMonth(anchorMs, 1) + 7 * DAY_MS };
}
  ...
  return function CalendarPanelTab() {
    ...
    async function loadMonth(nextAnchor) {
      ...
      try {
        ...
        const [calendarsOut, range, draftItems] = await Promise.all([api.calendars(), api.eventsInRange(win.fromMs, win.toMs), api.drafts()]);
        setCalendars(calendarsOut.calendars);
        setEvents(range.events);
        if (range.error) setError(range.error);
    ...
    const subtitle = loading ? "\u6B63\u5728\u8BFB\u53D6\u65E5\u5386\u2026" : `${formatDayLabel(selectedDayMs, Date.now())} \xB7 ${dayEvents.length === 0 ? "\u65E0\u5B89\u6392" : `${dayEvents.length} \u573A\u5B89\u6392`}`;
    return el(
      "div",
            ...
            ...dayEvents.map((event, index) => {
              ...
              return el(
                ...
                expanded ? el(
                  ...
                  el("span", { className: "text-caption-1-regular text-text-secondary" }, `\u63D0\u9192\uFF1A${alarmTimesText(event)}`),
...
function activate(ctx) {
  ...
  const api = {
    ...
    eventsInRange: async (fromMs, toMs) => {
      try {
        const outcome = await apple.listEvents({ calendar: null, fromMs, toMs, maxRows: MAX_EVENT_ROWS });
