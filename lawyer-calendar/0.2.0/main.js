...
function listEventsScript(input) {
  const body = [
    ...
    "    }",
    '    let location = "";',
    '    try { location = String(ev.location() || ""); } catch (e3) { location = ""; }',
    '    let notes = "";',
    '    try { notes = String(ev.description() || ""); } catch (e4) { notes = ""; }',
    "    let isRecurring = false;",
    "    try {",
    "      const r = ev.recurrence();",
    '      isRecurring = !(r === null || r === undefined || r === "");',
    "    } catch (e5) { isRecurring = false; }",
...
function groupEventsByDay(events, fromMs, toMs) {
  const buckets = [];
  for (let dayStart = beijingDayStart(fromMs); dayStart < toMs; dayStart += DAY_MS) {
    buckets.push({ dayStartMs: dayStart, events: [] });
  }
  for (const event of [...events].sort((a, b) => a.startMs - b.startMs)) {
    if (!eventOverlapsWindow(event, fromMs, toMs)) continue;
    for (const bucket of buckets) {
      const dayEnd = bucket.dayStartMs + DAY_MS;
      if (event.startMs < dayEnd && event.endMs > bucket.dayStartMs) bucket.events.push(event);
    }
  }
  return buckets;
}
...
function weekdayOf(dayStartMs) {
  ...
  return WEEKDAY_NAMES[new Date(Date.UTC(t.y, t.mo - 1, t.d)).getUTCDay()];
}
var pad22 = (value) => String(value).padStart(2, "0");
function formatDayLabel(dayStartMs, nowMs) {
  const t = beijingWallClockParts(dayStartMs);
  const base = `${t.mo}\u6708${t.d}\u65E5 ${weekdayOf(dayStartMs)}`;
  ...
  return function CalendarPanelTab() {
    ...
    async function refresh() {
      ...
      try {
        ...
        setWin({ fromMs: upcoming.fromMs, toMs: upcoming.toMs });
        if (upcoming.error) setError(upcoming.error);
    ...
    const buckets = win ? groupEventsByDay(visible, win.fromMs, win.toMs) : [];
    const subtitle = loading ? "\u6B63\u5728\u8BFB\u53D6\u65E5\u5386\u2026" : error !== "" && events.length === 0 ? "\u65E5\u5386.app \u6682\u4E0D\u53EF\u7528" : activeCalendar === "" ? `\u672A\u6765 7 \u5929 ${events.length} \u573A` : `\u300A${activeCalendar}\u300B\u672A\u6765 7 \u5929 ${visible.length} \u573A`;
    return el(
      "div",
      ...
      el(
        ...
        !loading && visible.length === 0 && error === "" ? el(
          "div",
          { className: "flex flex-col items-center text-center", style: { padding: "56px 24px 0" } },
          el("div", {
          ...
          }, icon(el, "calendar", { size: 26 })),
          el("div", { className: "text-body-2-medium text-text-primary", style: { marginTop: "12px" } }, "\u672A\u6765 7 \u5929\u6CA1\u6709\u65E5\u7A0B"),
          el("div", { className: "mt-1 text-caption-1-regular text-text-tertiary" }, "\u8BA9 AI \u5E2E\u4F60\u5B89\u6392\u5EAD\u5BA1\u3001\u4F1A\u89C1\u6216\u671F\u9650\uFF0C\u786E\u8BA4\u540E\u4F1A\u51FA\u73B0\u5728\u8FD9\u91CC\u3002")
        ) : el(
          "div",
          { className: "flex flex-col" },
          ...buckets.map((bucket) => {
            const nowMs = win?.fromMs ?? Date.now();
            const label = formatDayLabel(bucket.dayStartMs, nowMs);
            const isToday = label.startsWith("\u4ECA\u5929");
            return el(
              "div",
              { key: bucket.dayStartMs },
              el("div", {
                className: `px-3 pb-1 text-caption-2-medium ${isToday ? "text-status-blue-text" : "text-text-tertiary"}`,
                style: { paddingTop: "10px" }
              }, label),
              bucket.events.length === 0 ? el("div", {
                className: "px-3 pb-1 text-caption-2-medium opacity-60",
                ...
                ...bucket.events.map(
                  (event, index) => el(
                    "div",
                    {
                      key: `${event.uid || event.startMs}-${event.title}-${index}`,
                      className: "flex items-start gap-2 rounded-xl py-2 transition-colors hover:bg-background-secondary-hover",
                      style: { paddingInline: "8px" }
                    },
                    // 时间列
                    el("span", {
                      className: `shrink-0 text-caption-1-medium ${isToday ? "text-status-blue-text" : "text-text-secondary"}`,
                    ...
                    // 提醒图标
                    event.alarmTimes.length > 0 || event.hasAlarm ? el(
                      "span",
          ...
          ...drafts.map(
            (item) => el(
              ...
              el(
                ...
                el("button", {
                  type: "button",
                  className: "flex items-center gap-1 rounded-full bg-blue-600 px-3 text-caption-2-medium text-white shadow-sm transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50",
