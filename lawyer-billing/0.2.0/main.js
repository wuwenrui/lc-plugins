...
function renderStatusText(snapshot, daily, refreshedAt) {
  ...
  if (snapshot.balanceYuan === null) {
    ...
    lines.push(
      note === "" ? `\u6A21\u578B\u7AD9\u4F59\u989D\uFF1A${formatYuan(snapshot.balanceYuan)}` : `\u6A21\u578B\u7AD9\u4F59\u989D\uFF1A${formatYuan(snapshot.balanceYuan)}\uFF08${note}\uFF09`
    );
  }
  ...
  return el(
    ...
    el(
      ...
      el(
        ...
        `\u5408\u8BA1 ${formatYuan(total.yuan)} \xB7 ${total.calls} \u6B21`
      )
    ),
    el(
      "div",
      ...
      ...daily.map((day) => {
        ...
        return el(
          ...
          {
            key: day.date,
            title: `${day.date}\uFF1A${day.calls > 0 ? `${formatYuan(day.yuan)}\uFF08${day.calls} \u6B21\u8C03\u7528\uFF09` : "\u65E0\u6D88\u8D39"}`,
...
function createBillingPanelTab(ctx, deps) {
  ...
  return function BillingPanelTab() {
    ...
    const month = data === null ? null : summarizeMonth(data.rows, new Date(data.at), data.billing.quotaPerUnit, data.billing.exchangeRate);
    const tokenLeftYuan = snapshot === null || snapshot.tokenQuota === null ? null : quotaToYuan(snapshot.tokenQuota, snapshot.quotaPerUnit, snapshot.exchangeRate);
    const balance = snapshot === null ? null : snapshot.balanceYuan;
    const low = balance !== null && balance < LOW_BALANCE_YUAN;
      ...
      el(
        ...
        el(
          ...
          data === null ? null : el(
            ...
            el("span", { className: "text-caption-1-medium text-text-tertiary" }, "\u6A21\u578B\u7AD9\u4F59\u989D"),
            el(
              "div",
              ...
              el(
                ...
                balance === null ? "\u4E0D\u9650\u989D" : formatYuan(balance)
              ),
            ...
            el(
              ...
              tokenLeftYuan === null ? `\u4EE4\u724C\u7D2F\u8BA1\u5DF2\u7528 ${formatYuan(data.billing.usedYuan)} \xB7 \u4EE4\u724C\u4E0D\u9650\u989D` : `\u4EE4\u724C\u7D2F\u8BA1\u5DF2\u7528 ${formatYuan(data.billing.usedYuan)} \xB7 \u4EE4\u724C\u5269\u4F59 ${formatYuan(tokenLeftYuan)}`
            )
          ),
          data === null ? null : el("div", { className: "rounded-xl border-separator-border p-2.5", style: BORDER_STYLE }, renderDailyChart(el, daily)),
          data === null || month === null ? null : el(
            ...
            el(
              ...
              metricCard(el, "\u4ECA\u65E5\u6D88\u8D39", formatYuan(today === null ? 0 : today.yuan), `${today === null ? 0 : today.calls} \u6B21\u8C03\u7528`),
              metricCard(el, "\u672C\u6708\u6D88\u8D39", formatYuan(month.yuan), `${month.calls} \u6B21\u8C03\u7528`)
            ),
            el(
              "div",
              { className: "flex gap-1.5" },
              metricCard(el, "\u8FD1 7 \u5929\u6D88\u8D39", formatYuan(total.yuan), `${total.calls} \u6B21\u8C03\u7528`),
              metricCard(
                el,
                "\u4EE4\u724C\u7D2F\u8BA1\u5DF2\u7528",
                formatYuan(data.billing.usedYuan),
                tokenLeftYuan === null ? "\u4EE4\u724C\u4E0D\u9650\u989D" : `\u4EE4\u724C\u5269\u4F59 ${formatYuan(tokenLeftYuan)}`
              )
            )
          ),
          el(
  ...
  if (typeof ctx.ui.registerPanelTab === "function") {
    disposers.push(
      ctx.ui.registerPanelTab({
        key: SETTINGS_KEY,
        label: () => "\u7528\u91CF",
        component: createBillingPanelTab(ctx, { loadToken, refresh, openSettings: () => openSettings(ctx) })
      })
    );
  }
  return () => {
...
function createStatsCard(createElement, data) {
  ...
  const balanceLow = balance !== null && balance < LOW_BALANCE_YUAN;
  const note = balanceUsdNote(snapshot);
  const tokenLeftYuan = snapshot.tokenQuota === null ? null : quotaToYuan(snapshot.tokenQuota, snapshot.quotaPerUnit, snapshot.exchangeRate);
  return createElement(
    "section",
    ...
    createElement(
      ...
      createElement(
        "div",
        { className: "flex flex-col gap-1.5" },
        createElement("span", { className: "text-caption-1-medium text-text-tertiary" }, "\u5F53\u524D\u4F59\u989D\uFF08\u4EBA\u6C11\u5E01\uFF09"),
        createElement(
          "span",
          {
          ...
          balance === null ? "\u4E0D\u9650\u989D\u4EE4\u724C" : formatYuan(balance)
        ),
    ...
    createElement(
      "span",
      { className: "text-caption-1-regular text-text-secondary" },
      tokenLeftYuan === null ? `\u4EE4\u724C\u7D2F\u8BA1\u5DF2\u7528 ${formatYuan(snapshot.usedYuan)} \xB7 \u4EE4\u724C\u4E0D\u9650\u989D` : `\u4EE4\u724C\u7D2F\u8BA1\u5DF2\u7528 ${formatYuan(snapshot.usedYuan)} \xB7 \u4EE4\u724C\u5269\u4F59 ${formatYuan(tokenLeftYuan)}`
    ),
    ...
    renderDailyChart(createElement, data.daily),
    createElement(
      "p",
      { className: "text-caption-2-medium text-text-tertiary" },
...
function createStatusPill(ctx, deps) {
  ...
  return function BillingStatusPill() {
    ...
    const className = state.tone === "low" ? "rounded-full bg-background-tertiary-error text-caption-2-medium text-text-error-primary" : state.tone === "muted" ? "rounded-full bg-badge-neutral-background text-caption-2-medium text-text-tertiary transition-colors hover:bg-background-secondary-hover" : "rounded-full bg-badge-neutral-background text-caption-2-medium text-text-secondary transition-colors hover:bg-background-secondary-hover";
    return createElement(
      "button",
      { type: "button", className, title: state.title, style: { padding: "2px 10px" }, onClick: () => deps.openSettings() },
      state.text
    );
  };
}
