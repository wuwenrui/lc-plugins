...
async function runMailCommand(ctx, cmd, args = {}, timeoutMs = DEFAULT_EXEC_TIMEOUT_MS) {
  ...
  const request = encodeRequest(id, cmd, args);
  if (new TextEncoder().encode(request).length > MAX_REQUEST_BYTES) {
    throw new MailServerFailure("args", "\u90AE\u4EF6\u8BF7\u6C42\u8D85\u8FC7 1 MiB\uFF0C\u5C1A\u672A\u6267\u884C");
  }
  const candidates = serverScriptCandidates(ctx);
  const script = await discoverScript(ctx, candidates);
  if (script === null) {
    throw new MailServerFailure("config", "\u672A\u627E\u5230\u53EF\u8BFB\u53D6\u7684\u90AE\u4EF6\u7EC4\u4EF6\uFF0C\u5C1A\u672A\u6267\u884C\uFF1B\u8BF7\u91CD\u65B0\u5B89\u88C5\u90AE\u4EF6\u63D2\u4EF6");
  }
  let outcome;
  try {
    outcome = await invoke.call(ctx.bridge, "plugin_exec_run", {
      bin: "node",
      args: [script, "--stdin-once"],
      stdin: request,
      timeoutMs: Math.min(timeoutMs, DEFAULT_EXEC_TIMEOUT_MS)
    });
  } catch {
    throw uncertainResult(cmd);
  }
  const response = outcome?.code === 0 ? parseResponse(String(outcome.stdout ?? "")) : null;
  if (response === null || response.id !== id) throw uncertainResult(cmd);
  if (response.ok) return response.result;
  if (cmd === "send" && !["args", "auth", "policy", "config"].includes(response.error.code)) {
    throw uncertainResult(cmd);
  }
  throw new MailServerFailure(response.error.code, response.error.message);
}
async function discoverScript(ctx, candidates) {
  for (const script of candidates) {
    try {
      const probe = await ctx.bridge.invoke("plugin_exec_run", {
        bin: "node",
        args: ["--check", script],
        timeoutMs: 5e3
      });
      if (probe?.code === 0) return script;
    } catch {
    }
  }
  return null;
}
function uncertainResult(cmd) {
  return new MailServerFailure("unknown", cmd === "send" ? "\u53D1\u4FE1\u7ED3\u679C\u672A\u77E5\uFF0C\u90AE\u4EF6\u53EF\u80FD\u5DF2\u88AB\u63A5\u6536\uFF1B\u7CFB\u7EDF\u672A\u81EA\u52A8\u91CD\u53D1\u3002\u8BF7\u5148\u5230\u90AE\u7BB1\u6838\u5B9E\uFF0C\u52FF\u76F4\u63A5\u518D\u6B21\u53D1\u9001" : "\u90AE\u4EF6\u64CD\u4F5C\u7ED3\u679C\u672A\u77E5\uFF0C\u672A\u81EA\u52A8\u91CD\u590D\u6267\u884C\uFF1B\u8BF7\u6838\u5B9E\u540E\u518D\u64CD\u4F5C");
}
...
function notifyAccountChange(ctx, event) {
  const listeners = channels.get(ctx.storage);
  if (listeners === void 0 || listeners.size === 0) return;
...
function subscribeAccountChange(ctx, listener) {
...
var storageChains = /* @__PURE__ */ new WeakMap();
function enqueueAccountTask(ctx, task) {
  const key = ctx.storage;
  const previous = storageChains.get(key);
  const run = previous === void 0 ? task() : previous.then(task, task);
  storageChains.set(key, run.then(
    () => void 0,
    () => void 0
  ));
  return run;
}
function storageErrorText(cause) {
  if (cause instanceof Error && cause.message.trim() !== "") return cause.message;
  return String(cause);
}
async function applyStorageMutations(ctx, mutations, uncertainEmail) {
  const snapshots = /* @__PURE__ */ new Map();
  for (const mutation of mutations) {
    if (!snapshots.has(mutation.key)) {
      snapshots.set(mutation.key, await ctx.storage.get(mutation.key));
    }
  }
  const attempted = [];
  const attemptedSet = /* @__PURE__ */ new Set();
  try {
    for (const mutation of mutations) {
      if (!attemptedSet.has(mutation.key)) {
        attempted.push(mutation.key);
        attemptedSet.add(mutation.key);
      }
      if (mutation.kind === "set") await ctx.storage.set(mutation.key, mutation.value);
      else await ctx.storage.delete(mutation.key);
    }
  } catch (cause) {
    ...
    for (const key of [...attempted].reverse()) {
      try {
        ...
        if (previous === void 0) await ctx.storage.delete(key);
        else await ctx.storage.set(key, previous);
      } catch (rollbackCause) {
        rollbackErrors.push(`${key}: ${storageErrorText(rollbackCause)}`);
      }
    }
    if (rollbackErrors.length === 0) throw cause;
    notifyAccountChange(ctx, { kind: "reload", email: uncertainEmail });
    const error = new Error(
      `\u8D26\u53F7\u5B58\u50A8\u5199\u5165\u5931\u8D25\uFF0C\u4E14\u56DE\u6EDA\u672A\u5B8C\u6210\uFF1B\u5F53\u524D\u72B6\u6001\u672A\u77E5\uFF0C\u8BF7\u91CD\u65B0\u8BFB\u53D6\u8D26\u53F7\u8BBE\u7F6E\u3002\u539F\u9519\u8BEF\uFF1A${storageErrorText(cause)}\uFF1B\u56DE\u6EDA\u9519\u8BEF\uFF1A${rollbackErrors.join("\uFF1B")}`,
      { cause }
    );
    error.name = "AccountStorageStateUnknownError";
    throw error;
  }
}
async function loadAccounts(ctx) {
  return enqueueAccountTask(ctx, () => loadAccountsLocked(ctx));
}
async function loadAccountsLocked(ctx) {
  ...
  if (isBoundAccount(legacy)) {
    ...
    const mutations = [
      { kind: "set", key: STORAGE_ACCOUNTS_KEY, value: [legacy] },
      { kind: "set", key: STORAGE_ACTIVE_KEY, value: legacy.email }
    ];
    if (legacyStatus !== void 0 && legacyStatus !== null) {
      const statuses = await getStatuses(ctx);
      mutations.push({
        kind: "set",
        key: STORAGE_STATUSES_KEY,
        value: { ...statuses, [legacy.email]: legacyStatus }
      });
    }
    mutations.push(
      { kind: "delete", key: LEGACY_ACCOUNT_KEY },
      { kind: "delete", key: LEGACY_STATUS_KEY }
    );
    await applyStorageMutations(ctx, mutations, legacy.email);
    return [legacy];
  }
  return [];
}
async function pickAccount(ctx, email) {
  return enqueueAccountTask(ctx, () => pickAccountLocked(ctx, email));
}
async function pickAccountLocked(ctx, email) {
  const list = await loadAccountsLocked(ctx);
  if (list.length === 0) {
  ...
  const active = await getActiveEmailLocked(ctx);
  return list.find((item) => item.email === active) ?? list[0];
}
async function getActiveEmail(ctx) {
  return enqueueAccountTask(ctx, () => getActiveEmailLocked(ctx));
}
async function getActiveEmailLocked(ctx) {
  const list = await loadAccountsLocked(ctx);
...
async function upsertAccount(ctx, account) {
  return enqueueAccountTask(ctx, () => upsertAccountLocked(ctx, account));
}
async function upsertAccountLocked(ctx, account) {
  const list = await loadAccountsLocked(ctx);
  const next = [account, ...list.filter((item) => item.email !== account.email)];
  await applyStorageMutations(
    ctx,
    [
      { kind: "set", key: STORAGE_ACCOUNTS_KEY, value: next },
      { kind: "set", key: STORAGE_ACTIVE_KEY, value: account.email }
    ],
    account.email
  );
  return next;
...
async function removeAccount(ctx, email) {
  return enqueueAccountTask(ctx, () => removeAccountLocked(ctx, email));
}
async function removeAccountLocked(ctx, email) {
  const list = await loadAccountsLocked(ctx);
  const next = list.filter((item) => item.email !== email);
  const storedActive = await ctx.storage.get(STORAGE_ACTIVE_KEY);
  const active = typeof storedActive === "string" && list.some((item) => item.email === storedActive) ? storedActive : list[0]?.email ?? null;
  const nextActive = active === email || !next.some((item) => item.email === active) ? next[0]?.email ?? "" : active;
  const statuses = await getStatuses(ctx);
  const nextStatuses = { ...statuses };
  delete nextStatuses[email];
  await applyStorageMutations(
    ctx,
    [
      { kind: "set", key: STORAGE_ACCOUNTS_KEY, value: next },
      { kind: "set", key: STORAGE_ACTIVE_KEY, value: nextActive },
      { kind: "set", key: STORAGE_STATUSES_KEY, value: nextStatuses }
    ],
    email
  );
  return next;
...
async function setActiveEmail(ctx, email) {
  return enqueueAccountTask(ctx, () => setActiveEmailLocked(ctx, email));
}
async function setActiveEmailLocked(ctx, email) {
  const list = await loadAccountsLocked(ctx);
  ...
  await applyStorageMutations(
    ctx,
    [{ kind: "set", key: STORAGE_ACTIVE_KEY, value: email }],
    email
  );
}
...
async function setStatus(ctx, email, status) {
  return enqueueAccountTask(ctx, () => setStatusLocked(ctx, email, status));
}
async function setStatusLocked(ctx, email, status) {
  const statuses = await getStatuses(ctx);
  await ctx.storage.set(STORAGE_STATUSES_KEY, { ...statuses, [email]: status });
}
...
function sameAccountConfig(a, b) {
  if (a === null || b === null) return a === b;
  return a.email === b.email && a.authCode === b.authCode && a.imap.host === b.imap.host && a.imap.port === b.imap.port && a.imap.secure === b.imap.secure && a.smtp.host === b.smtp.host && a.smtp.port === b.smtp.port && a.smtp.secure === b.smtp.secure;
}
function buildAiDraft(selected, email) {
  ...
  for (const [folder, items] of byFolder) {
    ...
    sections.push(`\u3010${folderDisplayName(folder)}\u3011\uFF08\u539F\u59CB\u90AE\u4EF6\u5939\u540D\uFF1A${JSON.stringify(folder)}\uFF09
${lines.join("\n")}`);
  }
  return [
    ...
    "1. \u9010\u5C01\u7528 mail_read \u8BFB\u53D6\u6B63\u6587\uFF08folder \u53C2\u6570\u7528\u5404\u5206\u7EC4\u6807\u6CE8\u7684\u539F\u59CB\u90AE\u4EF6\u5939\u540D\uFF0Cuid \u4E00\u4E00\u5BF9\u5E94\uFF1B\u90AE\u4EF6\u670D\u52A1\u5668\u53EA\u8BA4\u539F\u59CB\u540D\uFF0C\u4E0D\u8BA4\u4E2D\u6587\u8BD1\u540D\uFF09\uFF1B",
...
function createMailPanelTab(ctx) {
  ...
  return function MailPanelTab() {
    ...
    guard.reload = reloadConfig;
    async function reloadConfig() {
      const epoch = ++guard.configEpoch;
      const baseline = guard.email;
      setConfigState("loading");
      let list;
      let activeEmail;
      try {
        list = await loadAccounts(ctx);
        activeEmail = await getActiveEmail(ctx);
      } catch (cause) {
        if (guard.configEpoch !== epoch || guard.email !== baseline) return;
      ...
      const next = list.find((item) => item.email === activeEmail) ?? list[0] ?? null;
      const sameEmail = account !== null && next !== null && account.email === next.email;
      const configsChanged = accounts.length !== list.length || accounts.some((previous) => !sameAccountConfig(previous, list.find((item) => item.email === previous.email) ?? null));
      guard.generation++;
      guard.accountGeneration++;
      if (configsChanged || !sameAccountConfig(account, next)) {
        guard.contentEpoch++;
        setSelection({});
        setDetails({});
        setDownloadState({});
    ...
    async function loadFolderList(bound, generation) {
      const owner = bound.email;
      const result = await runMailCommand(ctx, "list_folders", credentialArgs(bound));
      if (guard.email !== owner || guard.accountGeneration !== generation) return null;
      const selectable = result.folders.filter((item) => item.selectable);
    ...
    useEffect(() => {
      void reloadConfig();
      const unsubscribe = subscribeAccountChange(ctx, () => {
        setNotice("");
        void guard.reload();
      });
      return () => {
    ...
    useEffect(() => {
      if (account === null) return;
      const owner = account.email;
      const generation = guard.accountGeneration;
      const messageGeneration = guard.generation;
      setReadState("loading");
      void (async () => {
        try {
          const selectable = await loadFolderList(account, generation);
          if (selectable === null) return;
          if (guard.email !== owner || guard.accountGeneration !== generation) return;
          const target = guard.desiredFolder;
          const finalTarget = selectable.some((item) => item.name === target) ? target : "INBOX";
          ...
          guard.desiredFolder = finalTarget;
          setFolder(finalTarget);
          await loadMessages(finalTarget);
        } catch (cause) {
          if (guard.email !== owner || guard.accountGeneration !== generation) return;
          if (guard.generation !== messageGeneration) {
            setNotice(`\u90AE\u4EF6\u5939\u5237\u65B0\u5931\u8D25\uFF1A${errorText(cause)}\uFF1B\u5F53\u524D\u90AE\u4EF6\u5217\u8868\u4FDD\u6301\u4E0D\u53D8\u3002`);
            return;
          }
          setError(errorText(cause));
          setReadState("failed");
    ...
    async function onExpand(uid) {
      ...
      setNotice("");
      setExpanded(key);
      if (details[key]?.detail !== void 0) return;
      const epoch = guard.contentEpoch;
      if (guard.pendingReads.get(key) === epoch) return;
      guard.pendingReads.set(key, epoch);
      setDetails((prev) => ({ ...prev, [key]: {} }));
      try {
        const detail = await runMailCommand(ctx, "read", credentialArgs(account, { uid, folder }));
        if (guard.contentEpoch !== epoch) return;
        setDetails((prev) => ({ ...prev, [key]: { detail } }));
      } catch (cause) {
        if (guard.contentEpoch !== epoch) return;
        setDetails((prev) => ({ ...prev, [key]: { error: errorText(cause) } }));
      } finally {
        if (guard.pendingReads.get(key) === epoch) guard.pendingReads.delete(key);
      }
    }
    ...
    if (configState === "loading" && account === null) {
      return el(
        "div",
    ...
    if (configState === "failed" && account === null) {
      return el(
        "div",
        { className: "flex h-full flex-col items-center justify-center gap-3 px-8 text-center" },
        el("p", { className: "text-caption-1-medium text-text-error-primary" }, "\u8BFB\u53D6\u90AE\u7BB1\u914D\u7F6E\u5931\u8D25"),
        el("p", { className: "text-caption-2-regular text-text-tertiary" }, error),
        el("button", { className: "rounded-full bg-blue-600 px-4 py-2 text-caption-1-medium text-white", onClick: () => {
          void reloadConfig();
        } }, "\u91CD\u65B0\u8BFB\u53D6\u90AE\u7BB1\u914D\u7F6E")
      );
    }
...
${row.to}`.toLowerCase().includes(needle));
    ...
    return el(
      ...
      el(
        ...
        readState === "loading" ? "\u6B63\u5728\u53D6\u90AE\u4EF6\u2026" : readState === "failed" ? "\u90AE\u4EF6\u5C1A\u672A\u8BFB\u53D6\u6210\u529F" : total > rows.length ? `${folderDisplayName(folder)}\u5171 ${total} \u5C01 \xB7 \u663E\u793A\u6700\u65B0 ${rows.length} \u5C01${visible.length === rows.length ? "" : ` \xB7 \u7B5B\u51FA ${visible.length} \u5C01`}` : `${folderDisplayName(folder)}\u5171 ${rows.length} \u5C01${visible.length === rows.length ? "" : ` \xB7 \u7B5B\u51FA ${visible.length} \u5C01`}`
      ),
      // ———————— 邮件列表 ————————
      el(
        "div",
        ...
        visible.length === 0 ? el(
          ...
          el(
            ...
            readState === "loading" ? "\u6B63\u5728\u8BFB\u53D6\u90AE\u4EF6\u2026" : readState === "failed" ? "\u6682\u65F6\u65E0\u6CD5\u8BFB\u53D6\u90AE\u4EF6" : needle === "" ? "\u8FD9\u91CC\u8FD8\u5F88\u5B89\u9759" : "\u6CA1\u6709\u5339\u914D\u7684\u90AE\u4EF6"
          ),
          el(
            "div",
            { className: "mt-1 text-caption-1-regular text-text-tertiary" },
            readState === "loading" ? "\u8BF7\u7A0D\u5019\uFF0C\u6B63\u5728\u8FDE\u63A5\u90AE\u7BB1" : readState === "failed" ? "\u8BF7\u68C0\u67E5\u4E0A\u65B9\u63D0\u793A\u540E\u91CD\u8BD5" : needle === "" ? "\u6362\u4E2A\u90AE\u4EF6\u5939\u770B\u770B\uFF0C\u6216\u70B9\u53F3\u4E0A\u89D2\u5237\u65B0" : "\u6362\u4E2A\u5173\u952E\u8BCD\uFF0C\u6216\u6E05\u7A7A\u641C\u7D22\u518D\u8BD5"
          )
        ) : el(
          "div",
...
function createMailSettingsPanel(ctx) {
  ...
  return function MailSettingsPanel() {
    ...
    async function testOne(account) {
      return runMailCommand(ctx, "test", {
        email: account.email,
        authCode: account.authCode,
        imap: account.imap,
        smtp: account.smtp
    ...
    async function bind() {
      ...
      try {
        ...
        const detail = await testOne(candidate);
        await upsertAccount(ctx, candidate);
        notifyAccountChange(ctx, { kind: "upsert", email: candidate.email });
        setAuthCode("");
        ...
        try {
          await setStatus(ctx, candidate.email, {
            ok: true,
          ...
          followUpFailures.push(`\u8FDE\u63A5\u72B6\u6001\u4FDD\u5B58\u5931\u8D25\uFF1A${errorText(cause)}`);
        }
        try {
          await reload();
        } catch (cause) {
          followUpFailures.push(`\u8BBE\u7F6E\u9875\u5237\u65B0\u5931\u8D25\uFF1A${errorText(cause)}`);
        }
        if (followUpFailures.length > 0) {
          setError(
            `\u90AE\u7BB1 ${candidate.email} \u5DF2\u7ED1\u5B9A\u5E76\u8BBE\u4E3A\u5F53\u524D\uFF0C\u4F46${followUpFailures.join("\uFF1B")}\u3002\u53F3\u4FA7\u300C\u90AE\u4EF6\u300D\u9762\u677F\u5DF2\u6536\u5230\u5237\u65B0\u901A\u77E5\uFF0C\u8BF7\u91CD\u65B0\u6253\u5F00\u8BBE\u7F6E\u9875\u6838\u5BF9\u3002`
          );
        }
      } catch (cause) {
    ...
    async function testConnection(account) {
      ...
      try {
        let detail;
        try {
          detail = await testOne(account);
        } catch (cause) {
          const failed = { ok: false, checkedAt: (/* @__PURE__ */ new Date()).toISOString(), message: errorText(cause) };
          const followUpFailures2 = [];
          try {
            await setStatus(ctx, account.email, failed);
          } catch (statusCause) {
            followUpFailures2.push(`\u5931\u8D25\u72B6\u6001\u4FDD\u5B58\u5931\u8D25\uFF1A${errorText(statusCause)}`);
          }
          try {
            await reload();
          } catch (reloadCause) {
            followUpFailures2.push(`\u8BBE\u7F6E\u9875\u5237\u65B0\u5931\u8D25\uFF1A${errorText(reloadCause)}`);
          }
          setError(
            `\u6D4B\u8BD5\u672A\u901A\u8FC7\uFF08${account.email}\uFF09\uFF1A${failed.message}` + (followUpFailures2.length > 0 ? `\uFF1B${followUpFailures2.join("\uFF1B")}` : "")
          );
          return;
        }
        const followUpFailures = [];
        ...
        if (followUpFailures.length === 0) {
          ...
          setError(`\u6D4B\u8BD5\u901A\u8FC7\uFF1A${account.email} \u7684\u6536\u53D1\u8FDE\u63A5\u5747\u6B63\u5E38\uFF0C\u4F46${followUpFailures.join("\uFF1B")}\u3002`);
        }
      } finally {
        setBusy(false);
      }
    ...
    async function makeActive(account) {
      ...
      try {
        ...
        notifyAccountChange(ctx, { kind: "active", email: account.email });
        setNotice(`\u5F53\u524D\u4F7F\u7528\u90AE\u7BB1\u5DF2\u5207\u6362\u4E3A ${account.email}\uFF1A\u53F3\u4FA7\u300C\u90AE\u4EF6\u300D\u9762\u677F\u4E0E\u5BF9\u8BDD\u4E2D\u7684\u9ED8\u8BA4\u90AE\u7BB1\u64CD\u4F5C\u90FD\u4F7F\u7528\u5B83\u3002`);
        try {
          await reload();
        } catch (cause) {
          setError(
            `\u5F53\u524D\u4F7F\u7528\u90AE\u7BB1\u5DF2\u5207\u6362\u4E3A ${account.email}\uFF0C\u4F46\u8BBE\u7F6E\u9875\u5237\u65B0\u5931\u8D25\uFF1A${errorText(cause)}\u3002\u53F3\u4FA7\u300C\u90AE\u4EF6\u300D\u9762\u677F\u5DF2\u6536\u5230\u5237\u65B0\u901A\u77E5\uFF0C\u8BF7\u91CD\u65B0\u6253\u5F00\u8BBE\u7F6E\u9875\u6838\u5BF9\u3002`
          );
        }
      } catch (cause) {
        setError(errorText(cause));
    ...
    async function unbind(account) {
      ...
      try {
        ...
        notifyAccountChange(ctx, { kind: "remove", email: account.email });
        setNotice(
        ...
        try {
          ...
          setError(
            `\u5DF2\u89E3\u7ED1 ${account.email}\uFF0C\u4F46\u8BBE\u7F6E\u9875\u5237\u65B0\u5931\u8D25\uFF1A${errorText(cause)}\u3002\u53F3\u4FA7\u300C\u90AE\u4EF6\u300D\u9762\u677F\u5DF2\u6536\u5230\u5237\u65B0\u901A\u77E5\uFF0C\u8BF7\u91CD\u65B0\u6253\u5F00\u8BBE\u7F6E\u9875\u6838\u5BF9\u3002`
          );
        }
      } catch (cause) {
