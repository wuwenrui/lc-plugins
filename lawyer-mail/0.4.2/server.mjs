 ...
 *
 * 单次私密输入模式：`node server.mjs --stdin-once`，stdin 为一个 JSON 请求。
 ...
 * 收到 EOF 后才执行，上限 1 MiB；不接受 argv 请求或 JSONL 批处理。
 *
...
import { attachmentBytes, collectExactAttachments, sanitizeFilename } from './attachments.js'
import { saveAttachmentExclusive } from './attachment-save.js'
import { readPrivateRequest } from './private-input.js'
  ...
  return withImap(args, async (connection) => {
    ...
    const messages = page
      .map((uid) => {
        ...
        const parsed = parseMail(entry.data)
        const summary = summaries.get(uid)
        const flags = Array.isArray(summary?.flags) ? summary.flags.map((flag) => String(flag).toLowerCase()) : []
        return {
          uid,
          from: addressText(parsed.from),
          to: parsed.to.map(addressText).join(', '),
...
async function cmdSaveAttachments(args) {
  ...
  return withImap(args, async (connection) => {
    ...
    for (const item of targets) {
      ...
      const target = await saveAttachmentExclusive(dir, safeName, bytes)
      saved.push({
...
// Exactly one private request; wait for EOF before dispatch, never execute JSONL batches.
try {
  if (process.argv.length !== 3 || process.argv[2] !== '--stdin-once') {
    throw new Error('邮件组件仅接受私密输入模式，请升级宿主与插件')
  }
  const request = await readPrivateRequest(process.stdin)
  const response = await handleRequest(request)
  process.stdout.write(`${JSON.stringify(response)}\n`)
} catch {
  process.stdout.write(`${JSON.stringify({ id: 'unknown', ok: false, error: {
    code: 'args', message: '邮件请求输入无效或超限，未执行',
  } })}\n`)
  process.exitCode = 1
}
