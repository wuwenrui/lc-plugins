...
// src/panel.ts
var LIST_LIMIT = 50;
var BOX = "flex h-full min-h-0 flex-col gap-3 p-3 text-sm";
var INPUT_CLASS2 = "w-full rounded-md border border-gray-300 px-2 py-1.5 text-sm outline-none focus:border-blue-500 disabled:opacity-50";
var BUTTON_PRIMARY2 = "rounded-md bg-blue-600 px-3 py-1.5 text-xs font-medium text-white disabled:cursor-not-allowed disabled:opacity-50";
var BUTTON_PLAIN2 = "rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 disabled:cursor-not-allowed disabled:opacity-50";
var ROW_CLASS = "flex cursor-pointer items-start gap-2 rounded-md px-2 py-2 hover:bg-gray-50";
...
function selectionKey(folder, uid) {
  return `${folder}|${uid}`;
}
...
function buildAiDraft(selected) {
  ...
  for (const [folder, items] of byFolder) {
    const lines = items.map(
      (item) => `- [uid ${item.uid}] ${shortDate(item.date)}\uFF5C${item.from}\uFF5C${item.subject}`
    );
...
${lines.join("\n")}`);
  ...
  return [
    ...
    "",
    "\u5904\u7406\u8981\u6C42\uFF1A",
    "1. \u9010\u5C01\u7528 mail_read \u8BFB\u53D6\u6B63\u6587\uFF08folder \u7528\u4E0A\u9762\u5217\u51FA\u7684\u90AE\u4EF6\u5939\u540D\uFF0Cuid \u4E00\u4E00\u5BF9\u5E94\uFF09\uFF1B",
...
function createMailPanelTab(ctx) {
  ...
  return function MailPanelTab() {
    ...
    async function loadAccountOnly() {
      const saved = await ctx.storage.get(STORAGE_ACCOUNT_KEY);
      setAccount(isBoundAccount(saved) ? saved : null);
      setReady(true);
    }
    ...
    function onToggle(uid, row) {
      const key = selectionKey(folder, uid);
      setSelection((prev) => {
        const next = { ...prev };
        if (next[key] === void 0) {
    ...
    function onSelectVisible(visible2) {
      setSelection((prev) => {
        const next = { ...prev };
        const allSelected = visible2.every((row) => next[selectionKey(folder, row.uid)] !== void 0);
        for (const row of visible2) {
          const key = selectionKey(folder, row.uid);
    ...
    async function onExpand(uid) {
      ...
      try {
        const detail = await runMailCommand(ctx, "read", credentialArgs(account, {
          uid,
          folder
        }));
    ...
    async function onDownload(uid, filenames) {
      ...
      try {
        ...
        setDownloadState((prev) => ({
          ...prev,
          [`${key}|${tag}`]: `\u5DF2\u5B58\u5230\u300C\u4E0B\u8F7D\u300D\u6587\u4EF6\u5939\uFF1A${result.saved.map((item) => item.savedAs).join("\u3001")}`
        }));
      } catch (cause) {
        setDownloadState((prev) => ({ ...prev, [`${key}|${tag}`]: `\u4E0B\u8F7D\u5931\u8D25\uFF1A${errorText(cause)}` }));
      }
    }
    function onSendToAi() {
      ...
      ctx.composer.setDraft(buildAiDraft(selected));
      setNotice(`\u5DF2\u628A ${selected.length} \u5C01\u90AE\u4EF6\u5199\u8FDB\u804A\u5929\u8F93\u5165\u6846\uFF0C\u8BF7\u68C0\u67E5\u540E\u81EA\u5DF1\u70B9\u53D1\u9001\u3002`);
    }
    if (!ready) {
      return createElement("div", { className: BOX }, createElement("p", null, "\u6B63\u5728\u8BFB\u53D6\u90AE\u7BB1\u7ED1\u5B9A\u72B6\u6001\u2026"));
    }
    if (account === null) {
      return createElement(
        "div",
        { className: BOX },
        createElement("p", { className: "text-gray-600" }, "\u8FD8\u6CA1\u6709\u7ED1\u5B9A\u90AE\u7BB1\u3002\u7ED1\u5B9A\u540E\u53EF\u4EE5\u5728\u8FD9\u91CC\u6D4F\u89C8\u90AE\u4EF6\u3001\u591A\u9009\u4EA4\u7ED9 AI \u5904\u7406\u3001\u4E0B\u8F7D\u9644\u4EF6\u3002"),
        createElement(
          "button",
          {
            className: `${BUTTON_PRIMARY2} w-fit`,
            onClick: () => {
              try {
        ...
        error === "" ? null : createElement("p", { className: "text-xs text-red-600" }, error)
      );
    }
    const needle = keyword.trim().toLowerCase();
    const visible = needle === "" ? rows : rows.filter(
      (row) => `${row.subject}
${row.from}
${row.to}`.toLowerCase().includes(needle)
    );
    const selectedList = Object.values(selection);
    const expandedDetail = expanded === null ? void 0 : details[expanded];
    return createElement(
      "div",
      { className: BOX },
      // —— 工具行：文件夹 + 刷新 + 过滤 ——
      createElement(
        ...
        createElement(
          "select",
          {
            className: `${INPUT_CLASS2} max-w-[9rem]`,
            value: folder,
            onChange: (event) => onFolderChange(event.target.value)
          },
          ...folders.map(
            (item) => createElement("option", { key: item.name, value: item.name }, item.name)
          )
        ),
        createElement(
          "button",
          { className: BUTTON_PLAIN2, disabled: loading, onClick: () => void loadMessages(folder) },
          loading ? "\u52A0\u8F7D\u4E2D\u2026" : "\u5237\u65B0"
        ),
        createElement(
          "input",
      ...
      error === "" ? null : createElement("p", { className: "text-xs text-red-600" }, error),
      notice === "" ? null : createElement("p", { className: "text-xs text-blue-700" }, notice),
      // —— 邮件列表 ——
      createElement(
        "div",
        ...
        createElement(
          ...
          total > rows.length ? `\u8BE5\u5939\u5171 ${total} \u5C01\uFF0C\u663E\u793A\u6700\u65B0 ${rows.length} \u5C01` : `\u5171 ${rows.length} \u5C01`,
          visible.length === rows.length ? "" : `\uFF0C\u8FC7\u6EE4\u540E ${visible.length} \u5C01`
        ),
        visible.length === 0 ? createElement("p", { className: "px-2 py-4 text-gray-400" }, loading ? "\u6B63\u5728\u53D6\u90AE\u4EF6\u2026" : "\u6CA1\u6709\u90AE\u4EF6\u3002") : createElement(
          "div",
          null,
          ...visible.map((row) => {
            const key = selectionKey(folder, row.uid);
            ...
            return createElement(
              "div",
              { key, className: "border-b border-gray-100" },
              createElement(
                "div",
                {
                  className: ROW_CLASS,
                ...
                createElement(
                  ...
                  createElement(
                    ...
                    createElement("span", { className: "truncate font-medium" }, row.subject === "" ? "\uFF08\u65E0\u4E3B\u9898\uFF09" : row.subject),
                    createElement("span", { className: "shrink-0 text-[11px] text-gray-400" }, shortDate(row.date))
                  ),
                  createElement("div", { className: "truncate text-xs text-gray-500" }, row.from)
                )
              ),
              isOpen ? createElement(
                "div",
                { className: "mb-2 ml-6 rounded-md bg-gray-50 p-2 text-xs" },
                entry?.error !== void 0 ? createElement("p", { className: "text-red-600" }, entry.error) : entry?.detail === void 0 ? createElement("p", { className: "text-gray-400" }, "\u6B63\u5728\u8BFB\u4FE1\u2026") : renderDetail(createElement, entry.detail, key, downloadState, (filenames) => void onDownload(row.uid, filenames))
              ) : null
            );
          })
        )
      ...
      createElement(
        ...
        createElement(
          "button",
          { className: BUTTON_PLAIN2, disabled: visible.length === 0, onClick: () => onSelectVisible(visible) },
          selectedList.length > 0 && visible.every((row) => selection[selectionKey(folder, row.uid)] !== void 0) ? "\u53D6\u6D88\u672C\u9875\u5168\u9009" : "\u5168\u9009\u672C\u9875"
        ),
        createElement(
          "button",
          { className: BUTTON_PRIMARY2, disabled: selectedList.length === 0, onClick: onSendToAi },
          `\u4EA4\u7ED9 AI \u5904\u7406\uFF08${selectedList.length} \u5C01\uFF09`
        )
      )
    );
  };
}
function renderDetail(createElementFn, detail, key, downloadState, onDownload) {
  const el = createElementFn;
  const children = [
    el("p", { className: "text-gray-500" }, `\u6536\u4EF6\u4EBA\uFF1A${detail.to}${detail.cc === "" ? "" : `\uFF5C\u6284\u9001\uFF1A${detail.cc}`}`),
    el("p", { className: "text-gray-500" }, `\u65F6\u95F4\uFF1A${shortDate(detail.date)}`)
  ];
  if (detail.attachments.length > 0) {
    children.push(
      el(
        ...
        el("p", { className: "font-medium" }, `\u9644\u4EF6\uFF08${detail.attachments.length} \u4E2A\uFF09\uFF1A`),
        ...detail.attachments.map((item) => {
          const tag = item.filename;
          const state = downloadState[`${key}|${tag}`] ?? "";
          return el(
            "div",
            { key: tag, className: "flex items-center justify-between gap-2" },
            el("span", { className: "min-w-0 truncate" }, `${item.filename}\uFF08${Math.max(1, Math.round(item.size / 1024))}KB\uFF09`),
            el(
              "button",
              { className: `${BUTTON_PLAIN2} shrink-0 px-2 py-0.5`, onClick: () => onDownload([item.filename]) },
              "\u4E0B\u8F7D"
            ),
            state === "" ? null : el("span", { className: "shrink-0 text-[11px] text-gray-500" }, state)
          );
        }),
        detail.attachments.length > 1 ? el(
          "button",
          {
            className: `${BUTTON_PLAIN2} px-2 py-0.5`,
            onClick: () => onDownload(null)
          },
          "\u5168\u90E8\u4E0B\u8F7D"
        ) : null,
        downloadState[`${key}|*`] === void 0 ? null : el("p", { className: "text-[11px] text-gray-500" }, downloadState[`${key}|*`])
      )
    );
  }
  children.push(
    el(
      "div",
      { className: "mt-2 max-h-48 overflow-y-auto whitespace-pre-wrap rounded border border-gray-200 bg-white p-2" },
      detail.text === "" ? "\uFF08\u6B63\u6587\u4E3A\u7A7A\u6216\u53EA\u6709\u9644\u4EF6\uFF09" : detail.text
    ),
    el(
      "button",
      {
        className: `${BUTTON_PLAIN2} mt-2 w-fit px-2 py-0.5`,
        onClick: () => onDownload(null)
      },
      detail.attachments.length > 0 ? "\u628A\u5168\u90E8\u9644\u4EF6\u4E0B\u8F7D\u5230\u300C\u4E0B\u8F7D\u300D\u6587\u4EF6\u5939" : "\u8FD9\u5C01\u90AE\u4EF6\u6CA1\u6709\u9644\u4EF6"
    )
  );
  return el("div", { className: "space-y-1" }, ...children);
}
...
function isBoundAccount2(value) {
  if (typeof value !== "object" || value === null) return false;
  const candidate = value;
  return typeof candidate.email === "string" && candidate.email !== "" && typeof candidate.authCode === "string" && typeof candidate.imap === "object" && candidate.imap !== null && typeof candidate.smtp === "object" && candidate.smtp !== null;
}
async function requireAccount(ctx) {
  const saved = await ctx.storage.get(STORAGE_ACCOUNT_KEY);
  if (isBoundAccount2(saved)) return saved;
    ...
    ctx.tools.register({
      ...
      async execute() {
        return guarded(async () => {
          const saved = await ctx.storage.get(STORAGE_ACCOUNT_KEY);
          if (!isBoundAccount2(saved)) return NOT_BOUND_TEXT;
          ...
          lines.push("\u53EF\u7528\u5DE5\u5177\uFF1Amail_list_folders / mail_list_recent / mail_search / mail_read / mail_save_attachments / mail_send\uFF08\u9700\u786E\u8BA4\uFF09/ mail_draft_to_composer\u3002");
          return lines.join("\n");
        });
      }
    })
...
${read.text}`.trim();
  ...
  void (async () => {
    try {
      const saved = await ctx.storage.get(STORAGE_ACCOUNT_KEY);
      if (!isBoundAccount2(saved)) return;
      try {
