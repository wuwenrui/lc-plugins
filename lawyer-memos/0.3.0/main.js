...
try {
  ...
  if (rs.length > 0) { const r = rs[0]; hit = {list: String(r.container().name()), name: r.name(), completed: r.completed()} }
} catch (e) { hit = null }
JSON.stringify({ok: true, found: hit !== null, list: hit ? hit.list : null, name: hit ? hit.name : null, completed: hit ? hit.completed : null})`;
}
...
function toView(raw) {
  ...
  return {
    ...
    list: typeof record.list === "string" ? record.list : "",
...
function parseReminderList(result) {
  ...
  const record = result;
  if (record.ok !== true) throw new AppleSyncError(`readAllReminders \u5931\u8D25\uFF1A${String(record.error ?? "unknown")}`);
  if (!Array.isArray(record.items)) throw new AppleSyncError("readAllReminders \u7ED3\u679C\u7F3A\u5C11 items");
  const views = [];
...
function parseListNames(result) {
  ...
  const record = result;
  if (record.ok !== true) throw new AppleSyncError(`listLists \u5931\u8D25\uFF1A${String(record.error ?? "unknown")}`);
  if (!Array.isArray(record.names)) throw new AppleSyncError("listLists \u7ED3\u679C\u7F3A\u5C11 names");
  const names = [];
...
function parseFoundReminder(result) {
  ...
  const record = result;
  if (record.ok !== true) throw new AppleSyncError(`findReminder \u5931\u8D25\uFF1A${String(record.error ?? "unknown")}`);
  return {
...
function parseUpdatedOk(result) {
  ...
  const record = result;
  if (record.ok !== true) throw new AppleSyncError(`updateReminder \u5931\u8D25\uFF1A${String(record.error ?? "unknown")}`);
}
function parseDeleteResult(result) {
  ...
  throw new AppleSyncError(`deleteReminder \u5931\u8D25\uFF1A${String(record.error ?? "unknown")}`);
}
function validateAppleMap(value) {
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    if (record.list !== void 0 && (typeof record.list !== "string" || !record.list)) {
      throw new AppleSyncError(`apple-map \u6761\u76EE list \u4E0D\u5408\u6CD5\uFF1A${key}`);
    }
    map[key] = record.list === void 0 ? { id: record.id, name: record.name, lastCompleted: record.lastCompleted } : { id: record.id, name: record.name, lastCompleted: record.lastCompleted, list: record.list };
  }
  return map;
}
...
function parseTombstones(value) {
  ...
  for (const id of ids) {
    if (typeof id === "number" && Number.isSafeInteger(id) && id > 0) out.add(id);
  }
...
function activate(ctx) {
    ...
    return enqueueSync(async () => {
      try {
        ...
        for (const create of plan.appleCreates) {
          ...
          if (create.completeAfter) await osa(buildSetCompletedScript(id, true, create.list));
  ...
  function pushCreate(item) {
    ...
    void updateMap(async (map) => {
      ...
      return { ...map, [key]: { id, name: item.text, lastCompleted: false, list: listName } };
    }).catch(() => void 0);
  }
  function pushUpdate(item, recreate) {
    ...
    void updateMap(async (map) => {
      ...
      if (item.done) await osa(buildSetCompletedScript(id, true, listName));
      return { ...map, [key]: { id, name: item.text, lastCompleted: item.done, list: listName } };
    }).catch(() => void 0);
  }
  ...
  const api = {
    ...
    update: (id, patch) => mutate((current) => {
      const outcome = updateItem(current, id, patch);
      return { state: outcome.state, result: outcome };
    }),
    remove: (id) => mutate((current) => {
      const outcome = deleteItem(current, id);
      return { state: outcome.state, result: outcome.item };
    }),
    syncInfo: () => syncInfo(),
    syncNow: () => {
  ...
  if (typeof ctx.ui.registerPanelTab === "function") {
    disposers.push(
      ctx.ui.registerPanelTab({
        key: "lawyer-memos",
        label: () => "\u5907\u5FD8",
        component: createMemosPanelTab(ctx, api)
      })
    );
  }
  return () => {
