...
var MESSAGES = {
  ...
  host_incompatible: "\u5F53\u524D\u5BA2\u6237\u7AEF\u7684\u5B89\u5168\u6388\u6743\u56DE\u8C03\u4E0D\u53EF\u7528\uFF1B\u672C\u63D2\u4EF6\u4E0D\u4F1A\u56DE\u9000\u8BFB\u53D6\u672C\u673A\u914D\u7F6E\u6216\u6267\u884C Shell\u3002\u6B64\u5019\u9009\u7248\u8981\u6C42\u5C1A\u672A\u53D1\u5E03\u7684 LawyerCopilot 1.0.6 / \u63D2\u4EF6 SDK 0.4.3 \u6216\u66F4\u9AD8\u517C\u5BB9\u7248\u672C\u3002"
};
...
var CLIENT_INFO = { name: "LawyerCopilot-QCC", version: "0.2.1" };
...
function isVerifiedCompanyRecord(value) {
  if (!isRecord2(value)) return false;
  return nonEmptyText(value.company) && nonEmptyText(value.source);
}
...
function hasVerifiedToolPayload(result, server, tool) {
  if (server !== "company" || tool !== VERIFIED_COMPANY_TOOL) return false;
  if (hasVerifiedCompanyPayload(result.structuredContent)) return true;
  ...
  return result.content.some((item) => {
    ...
    try {
      return hasVerifiedCompanyPayload(JSON.parse(text));
    } catch {
      return false;
    }
  });
...
function recordCandidates(result) {
  ...
  for (const payload of resultPayloads(result)) {
    for (const record of companyRecords(payload)) {
      ...
      if (!current || cardScore(card) > cardScore(current.card)) byIdentity.set(identity, { card, token });
    }
  }
  ...
  return function QccPanelTab() {
    ...
    return el(
      ...
      el(
        ...
        { className: "px-3 pt-2" },
        el("div", {
          className: status.queryVerified ? "rounded-lg bg-background-secondary-default px-3 py-2 text-caption-1-regular text-text-secondary" : "rounded-lg bg-background-quaternary-default px-3 py-2 text-caption-1-regular text-text-secondary"
...
function createQccRuntime(context) {
  ...
  async function callQccTool(server, tool, args) {
    ...
    try {
      ...
      return renderCallText(server, SERVERS[server]?.label ?? server, tool, result, (/* @__PURE__ */ new Date()).toISOString(), exactSecrets);
    } catch (error) {
...
var HOST_LIMITATION_NOTE = "\u5019\u9009\u7248\u8981\u6C42\u5C1A\u672A\u53D1\u5E03\u7684 LawyerCopilot 1.0.6 / \u63D2\u4EF6 SDK 0.4.3\uFF1A\u7531\u5BBF\u4E3B\u63D0\u4F9B\u672C\u63D2\u4EF6\u4E13\u5C5E\u6388\u6743\u56DE\u8C03\u3001\u7981\u6B62\u7F51\u7EDC\u91CD\u5B9A\u5411\uFF0C\u5E76\u6309\u8BF7\u6C42\u7F16\u53F7\u6709\u754C\u8BFB\u53D6 MCP \u54CD\u5E94\u3002\u63D2\u4EF6\u6D88\u8D39\u4EE3\u7801\u5DF2\u63A5\u5165\uFF0C\u672C\u8F6E\u4ECD\u672A\u505A\u771F\u5B9E\u5BA2\u6237\u7AEF\u8054\u8C03\u3001\u767B\u5F55\u6216\u8D26\u53F7\u67E5\u8BE2\u3002";
function createSettingsPanel(ctx, api) {
  ...
  return function QccSettingsPanel() {
    ...
    const body = [
      createElement(
        "div",
        { className: "rounded-lg bg-background-tertiary-error px-3 py-2 text-caption-1-regular text-text-error-primary leading-relaxed" },
        HOST_LIMITATION_NOTE
      )
    ];
    if (!view) {
      body.push(
...
function activate(ctx) {
  ...
  disposers.push(
    ctx.tools.register({
      name: "qcc_list_tools",
      description: "\u5217\u51FA\u4F01\u67E5\u67E5\u5B98\u65B9 MCP \u5F53\u524D\u8D26\u53F7\u8FD4\u56DE\u4E14\u547D\u4E2D\u672C\u63D2\u4EF6\u670D\u52A1/\u5DE5\u5177\u7CBE\u786E\u540D\u5355\u7684\u53EA\u8BFB\u5DE5\u5177\u3002\u76EE\u5F55\u53EF\u8BFB\u4E0D\u4EE3\u8868\u67E5\u8BE2\u6743\u76CA\u3002",
      inputSchema: {
        type: "object",
        additionalProperties: false,
        properties: {
