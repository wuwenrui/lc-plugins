...
var CLIENT_INFO = { name: "LawyerCopilot-QCC", version: "0.2.5" };
...
function createQccPanelTab(ctx, api) {
  ...
  return function QccPanelTab() {
    ...
    const [retryKeyword, setRetryKeyword] = useState(null);
    ...
    async function runSearch(rawKeyword, recentLabel) {
      ...
      try {
        ...
        if (failure.code === "retry_confirmation_required") {
          setError("\u8FD9\u5BB6\u4F01\u4E1A\u4E0A\u6B21\u67E5\u8BE2\u7684\u7ED3\u679C\u672A\u786E\u8BA4\uFF0C\u53EF\u80FD\u5DF2\u7ECF\u8BA1\u8D39\u3002\u8BF7\u786E\u8BA4\u662F\u5426\u518D\u67E5\u4E00\u6B21\u3002");
          setRetryKeyword(query);
        } else {
    ...
    async function runCandidateSearch() {
      ...
      try {
        ...
        setCandidates(found);
        if (!found.length) setNotice("\u4F01\u67E5\u67E5\u6CA1\u6709\u8FD4\u56DE\u53EF\u786E\u8BA4\u7684\u4F01\u4E1A\u5019\u9009\uFF0C\u8BF7\u6362\u4E2A\u66F4\u5B8C\u6574\u7684\u4F01\u4E1A\u540D\u79F0\u6216\u7EDF\u4E00\u793E\u4F1A\u4FE1\u7528\u4EE3\u7801\u3002");
      } catch (cause) {
        const failure = publicError(cause);
        if (failure.code === "unsupported") setNotice("\u5F53\u524D\u4F01\u67E5\u67E5\u8D26\u53F7\u6CA1\u6709\u53EF\u7528\u7684\u76F8\u8FD1\u4F01\u4E1A\u641C\u7D22\uFF1B\u53EF\u5728\u7CFB\u7EDF\u6D4F\u89C8\u5668\u4F7F\u7528\u4F01\u67E5\u67E5\u5B98\u7F51\u67E5\u627E\u3002");
    ...
    async function confirmAndRetry() {
      const query = retryKeyword;
      if (!query || searching || retrying) return;
      setRetrying(true);
      try {
        await api.confirmQuickSearchRetry(query);
    ...
    return el(
      ...
      el(
        ...
        el(
          ...
          el("div", { className: "truncate text-caption-2-medium text-text-tertiary" }, "\u8D26\u53F7\u5DF2\u6388\u6743")
        ),
      ...
      error === "" ? null : el(
        ...
        retryKeyword === null ? null : el("button", {
          type: "button",
          className: "mt-2 rounded-lg border border-separator-border px-3 py-2 text-caption-1-medium text-text-primary disabled:opacity-50",
          disabled: searching || retrying,
          onClick: () => void confirmAndRetry()
        }, retrying ? "\u6B63\u5728\u91CD\u65B0\u67E5\u8BE2\u2026" : "\u4ECD\u8981\u91CD\u65B0\u67E5\u8BE2\uFF08\u53EF\u80FD\u518D\u6B21\u8BA1\u8D39\uFF09"),
...
function createQccRuntime(context) {
  ...
  async function confirmQuickSearchRetry(keyword) {
    const query = keyword.trim().slice(0, 60);
    if (!query) throw new QccError("args");
    await ensureLoaded();
    const authorizationId = grant?.authorizationId;
    if (!authorizationId) throw new QccError("unauthorized");
    const id = queryFingerprint(authorizationId, "company", PANEL_SEARCH_TOOLS[0], { searchKey: query });
    await clearQueryGuard(id, authorizationId);
  }
          ...
          createElement(
            ...
            ...view.servers.map(
              (item) => createElement(
                ...
                `${item.label} \xB7 ${item.status === "available" ? `${item.toolCount} \u4E2A\u53EA\u8BFB\u5DE5\u5177` : item.status === "unknown" ? "\u672A\u68C0\u67E5" : item.status === "empty" ? "\u672A\u53D1\u73B0\u53EF\u7528\u53EA\u8BFB\u5DE5\u5177" : "\u76EE\u5F55\u68C0\u67E5\u672A\u5B8C\u6210"}`
              )
            )
          ),
          createElement(
            ...
            "\u5206\u7C7B\u6570\u5B57\u662F\u5B98\u65B9\u76EE\u5F55\u4E2D\u672C\u63D2\u4EF6\u5141\u8BB8\u7684\u53EA\u8BFB\u5DE5\u5177\u6570\uFF0C\u4E0D\u662F\u63D2\u4EF6\u7684 AI \u5DE5\u5177\u603B\u6570\u3002AI \u53EF\u7528\u72B6\u6001\u3001\u6388\u6743\u3001\u76EE\u5F55\u548C\u53EA\u8BFB\u8C03\u7528\u56DB\u4E2A\u5165\u53E3\u3002"
          ),
          createElement(
            "div",
            ...
            createElement("button", {
              ...
              disabled: busy,
              onClick: () => void run(async () => {
                const result = await api.inspectDirectories();
                setMessage(`\u5DF2\u68C0\u67E5 ${result.checked} \u7C7B\u76EE\u5F55\uFF1B${result.failed} \u7C7B\u68C0\u67E5\u672A\u5B8C\u6210\u3002\u76EE\u5F55\u53EF\u8BFB\u4E0D\u4EE3\u8868\u67E5\u8BE2\u6743\u76CA\u3002`);
              })
            }, "\u68C0\u67E5\u5404\u7C7B\u53EF\u7528\u5DE5\u5177"),
            createElement(
              "p",
              { className: "text-caption-1-regular text-text-tertiary" },
              "\u53EA\u8BFB\u53D6\u5B98\u65B9\u5DE5\u5177\u76EE\u5F55\uFF0C\u4E0D\u53D1\u8D77\u4F01\u4E1A\u67E5\u8BE2\uFF1B\u7070\u8272\u201C\u672A\u68C0\u67E5\u201D\u4E0D\u4EE3\u8868\u6CA1\u6709\u6743\u9650\u3002"
            )
          ),
...
function activate(ctx) {
  ...
  disposers.push(
    ctx.tools.register({
      name: "qcc_list_tools",
      description: "\u5217\u51FA\u4F01\u67E5\u67E5\u5B98\u65B9 MCP \u5F53\u524D\u8D26\u53F7\u8FD4\u56DE\u3001\u7ECF\u8FC7\u53EA\u8BFB\u7B5B\u9009\u7684\u5DE5\u5177\u3002server=all \u9010\u7C7B\u68C0\u67E5 8 \u7C7B\u76EE\u5F55\uFF0C\u6307\u5B9A\u5355\u7C7B\u53EF\u67E5\u770B\u53C2\u6570 schema\uFF1B\u76EE\u5F55\u53EF\u8BFB\u4E0D\u4EE3\u8868\u67E5\u8BE2\u6743\u76CA\u3002",
      inputSchema: {
        type: "object",
        additionalProperties: false,
        properties: {
          server: { ...serverSchema, enum: [...ALL_SERVERS, "all"], description: `${serverSchema.description}\uFF1Ball \u53EA\u5217\u5404\u7C7B\u76EE\u5F55\u6458\u8981` },
      ...
      execute: (args = {}) => runTool(async () => {
        ...
        if (args?.server === "all") {
          const lines = [];
          for (const server2 of ALL_SERVERS) {
            try {
              const tools2 = filterReadOnlyTools(server2, await runtime.listServerTools(server2), keyword);
              lines.push(`${SERVERS[server2]?.label ?? server2}\uFF08${server2}\uFF09\uFF1A${tools2.length} \u4E2A\u53EA\u8BFB\u5DE5\u5177${tools2.length ? `\uFF1B${tools2.slice(0, 12).map((tool) => tool.name).join("\u3001")}${tools2.length > 12 ? "\u7B49" : ""}` : ""}`);
            } catch (cause) {
              lines.push(`${SERVERS[server2]?.label ?? server2}\uFF08${server2}\uFF09\uFF1A\u76EE\u5F55\u68C0\u67E5\u672A\u5B8C\u6210\uFF08${publicError(cause).message}\uFF09`);
            }
          }
          return textResult(`\u4F01\u67E5\u67E5\u5B98\u65B9 MCP \u76EE\u5F55\u6458\u8981\uFF1B\u53EA\u8BFB\u53D6\u5DE5\u5177\u76EE\u5F55\uFF0C\u672A\u53D1\u8D77\u4E1A\u52A1\u67E5\u8BE2\u3002\u76EE\u5F55\u53EF\u8BFB\u4E0D\u7B49\u4E8E\u53EF\u67E5\u8BE2\uFF1B\u8981\u67E5\u770B\u53C2\u6570\u8BF7\u6307\u5B9A server \u518D\u8C03\u7528 qcc_list_tools\u3002
${lines.join("\n")}`);
        }
  ...
  const settingsApi = {
    ...
    inspectDirectories: async () => {
      let checked = 0;
      let failed = 0;
      for (const server of ALL_SERVERS) {
        try {
          await runtime.listServerTools(server);
          checked += 1;
        } catch {
          failed += 1;
        }
  ...
  const panelApi = {
    ...
    confirmQuickSearchRetry: runtime.confirmQuickSearchRetry,
